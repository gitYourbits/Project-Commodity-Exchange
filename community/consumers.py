...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    def create_message(self, message_content, offering_id, notification_receiver_id):
        ...
        notification = Notification(parent = User.objects.filter(id=notification_receiver_id)[0], associated_url = f'/community/deal/{offering_id}by{self.rec[1]}/ongoing/', about=f'New message from {self.sender.username}<br><small>{message.message[:50]}...</small>')
